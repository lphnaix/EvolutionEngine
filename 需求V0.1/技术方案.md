...
- 坐标系：世界 XY 为地面平面，Z 为高度，正 Z 向上；1 tile ≈ 1 世界单位（约 1 米）；角度运算默认用弧度。
- 时间步长：逻辑目标 60 FPS，半固定步长，建议 clamp dt 在 1/90 ~ 1/30 秒并用累加器；渲染可变帧率，逻辑/渲染解耦。
- 随机/seed：地形生成、掉落、AI 随机数均接收 seed，可重现；避免使用全局静态 Random。存档中记录当前 seed。
- 配置/ID：JSON/YAML 均用 UTF-8；ID 统一用字符串、大小写敏感、不含空格，配置里用 ID 作为 key；路径使用正斜杠。
- 数值类型：运行时世界坐标/时间统一使用 float（单精度），配置/工具侧可用 double 做离线计算，但导出的值要落到 float 安全范围。
- 时间戳/时区：数据库中的 created_at/updated_at 使用 UTC（Unix 时间戳）；日志/存档都用同一基准，避免本地时区差异。
- 资源管线：静态配置与美术/音频分离；如果使用 MonoGame，建议 mgcb 生成 XNB，其它后端需提供等价的打包/加载方式。
