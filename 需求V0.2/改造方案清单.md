...
- [ ] 在 `需求V0.2/子系统设计概要.md` 新增「1.4 目标平台与硬件」章节，内容按《需求V0.2/技术方案.md》1.1 执行：OS=Windows 10+ x64，图形 API=MonoGame+DX11，输入=键鼠（手柄列为未来），推荐分辨率 1080p/1440p、最低 60FPS、最低硬件描述，非目标平台标注 v0.3+。
...
- [ ] 在 `需求V0.2/框架设计清单.md` 3.1 中明确：v0.2 使用正交相机，地形+实体用 Z-buffer+层级深度排序，不再依赖按 Y 排序。
- [ ] 在 `Engine.Rendering.Camera` 中实现正交相机矩阵（参考《需求V0.2/技术方案.md》2.1）。
